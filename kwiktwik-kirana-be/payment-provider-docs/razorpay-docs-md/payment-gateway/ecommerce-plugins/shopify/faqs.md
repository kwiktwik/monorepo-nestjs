<!-- Source: https://razorpay.com/docs/payments/payment-gateway/ecommerce-plugins/shopify/faqs -->

## Integration & Setup

1. How do I integrate 1 Razorpay with my Shopify store?

Follow the [build integration](/razorpay-docs-md/payment-gateway/ecommerce-plugins/shopify/integration-steps.md#1-build-integration) steps to integrate 1 Razorpay with your Shopify store.
...
1. When you try to reconnect, you will get the following screen. Click **Manage**.
   ...
   ![reconnect 1 Razorpay](https://razorpay.com/docs/payments/payment-gateway/ecommerce-plugins/shopify/build/browser/assets/images/razorpay-secure-reconnect.jpg)
...
3. Scroll down and click **Login**.
   ...
   **Handy Tips**
   ...
   ![Razorpay login](https://razorpay.com/docs/payments/payment-gateway/ecommerce-plugins/shopify/build/browser/assets/images/shopify-login-v2.gif)
4. Click **Activate** on the activation screen on your Shopify Dashboard.

   ![Shopify Authorize](https://razorpay.com/docs/payments/payment-gateway/ecommerce-plugins/shopify/build/browser/assets/images/shopify-1razorpay-activate.jpg)
...
- **Check for Integration with Another Account:** Verify if you have integrated your Shopify Store with a different Razorpay MID. If yes, revoke access under applications and retry the 1 Razorpay [integration steps](/razorpay-docs-md/payment-gateway/ecommerce-plugins/shopify/integration-steps.md#1-build-integration)  .
...
You can [update the email ID](/razorpay-docs-md/dashboard/account-settings/account-details.md#update-login-email) on Razorpay to match the one used in Shopify. Verify that the email ID associated with owner access on Razorpay matches the one used in Shopify.
...
3. Click Search by provider and type **Razorpay**. This will show all the Razorpay payment apps installed on your Shopify store. Click the Razorpay app that you wish to uninstall.
   ...
   ![Search Razorpay in Payment Providers](https://razorpay.com/docs/payments/payment-gateway/ecommerce-plugins/shopify/build/browser/assets/images/search-razorpay-payment-provider.jpg)
4. Scroll down to the bottom and click the **Uninstall** button to remove the app from your Shopify store.

   ![Uninstall the app](https://razorpay.com/docs/payments/payment-gateway/ecommerce-plugins/shopify/build/browser/assets/images/uninstall-app.jpg)
...
To avoid confusion, uninstall the legacy version of the plugin and [re-install](/razorpay-docs-md/payment-gateway/ecommerce-plugins/shopify/integration-steps.md#1-build-integration) the current plugin version (1 Razorpay).

9. Our Payment Gateway stopped displaying after migrating to Shopify. How can we reactivate it?

After migrating your website to Shopify, you need to integrate with the official Shopify 1 Razorpay plugin, as your previous Payment Gateway integration from the old platform will not be carried over. Follow the [integration steps](/razorpay-docs-md/payment-gateway/ecommerce-plugins/shopify/integration-steps.md) and once activated, your payment gateway will be displayed at checkout, allowing you to accept payments.
...
You can upgrade to 1 Razorpay by following a simple migration process. Know more about [how to migrate to 1 Razorpay](/razorpay-docs-md/payment-gateway/ecommerce-plugins/shopify/migration-steps.md).
...
![Shopify page payment error](https://razorpay.com/docs/payments/payment-gateway/ecommerce-plugins/shopify/build/browser/assets/images/econ-shopify-payment-error.jpg)
...
Follow the [build integration](/razorpay-docs-md/payment-gateway/ecommerce-plugins/shopify/integration-steps.md#1-build-integration) steps to install the plugin again.
...
You can test a payment for 1 Razorpay on the Shopify store by switching to test mode. Know more about [how to test a transaction in test mode.](/razorpay-docs-md/payment-gateway/ecommerce-plugins/shopify/integration-steps.md#21-make-a-test-transaction-in-test-mode)
...
Razorpay facilitates only payment collection. The partner is solely responsible for sending invoices or payment receipts with GST details to customers. To provide GST-compliant receipts, partners can use [Razorpay Invoice](/razorpay-docs-md/invoices.md) product when collecting payments.
...
No, Shopify currently supports only one Razorpay account integration per store. If you want to integrate a different Razorpay account, you need to [deactivate](/razorpay-docs-md/payment-gateway/ecommerce-plugins/shopify/faqs.md#7-how-do-i-uninstall-razorpay-payment-apps) the existing integration first and then [activate](/razorpay-docs-md/payment-gateway/ecommerce-plugins/shopify/integration-steps.md) the new account.
...
Razorpay does not provide a direct plugin integration for accepting payments through Meta Ads. However, you can use Razorpay Payment Pages with [Facebook Pixel and Google Tracking ID](/razorpay-docs-md/payment-pages/plugins-add-ons.md) to track payments and conversions from your Ad campaigns.

30. Can I use Razorpay Route API with my Shopify store?

No, Razorpay Route API is not supported for Shopify integration. Only the [1 Razorpay plugin](/razorpay-docs-md/payment-gateway/ecommerce-plugins/shopify/integration-steps.md) is supported for Shopify stores.
