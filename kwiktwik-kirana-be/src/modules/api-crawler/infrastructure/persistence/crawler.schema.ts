...
export const crawlEndpoints = pgTable(
  'crawl_endpoints',
  {
    ...
    // Pagination
    paginationType: varchar('pagination_type', { length: 50 }).notNull().default('none'),
    ...
    paginationStopOnEmpty: boolean('pagination_stop_on_empty').notNull().default(true),
    ...
    // Response
    responseContentHandling: varchar('response_content_handling', { length: 50 }).notNull().default('auto'),
    responseExtractFields: text('response_extract_fields').array(),
    responseMaxDbSizeBytes: integer('response_max_db_size_bytes').notNull().default(102400),
    responseStorage: varchar('response_storage', { length: 50 }).notNull().default('hybrid'),
    ...
    scheduleRunOnStart: boolean('schedule_run_on_start').notNull().default(false),
    
    // Rate Limit
    rateLimitRequestsPerMinute: integer('rate_limit_requests_per_minute').notNull().default(60),
    rateLimitMaxConcurrent: integer('rate_limit_max_concurrent').notNull().default(1),
    rateLimitDelayBetweenRequestsMs: integer('rate_limit_delay_between_requests_ms').notNull().default(1000),
    ...
    retryInitialDelayMs: integer('retry_initial_delay_ms').notNull().default(1000),
    retryOnStatusCodes: integer('retry_on_status_codes').array().default([429, 500, 502, 503, 504]),
    
    // Deduplication
    dedupEnabled: boolean('dedup_enabled').notNull().default(true),
    dedupKeyFields: text('dedup_key_fields').array().default(['url', 'method', 'query_params']),
...
export const crawlJobs = pgTable(
  ...
  (table) => ({
    ...
    dedupKeyIdx: uniqueIndex('crawl_jobs_dedup_key_idx')
      .on(table.dedupKey, table.endpointId),
    scheduledAtIdx: index('crawl_jobs_scheduled_at_idx').on(table.scheduledAt),
    statusPriorityCreatedIdx: index('crawl_jobs_status_priority_created_idx').on(
      table.status,
      table.priority,
      table.createdAt,
    ),
...
export const crawlResults = pgTable(
  'crawl_results',
  {
    ...
    // Storage location
    storageProvider: varchar('storage_provider', { length: 50 }).notNull().default('database'),
  ...
  (table) => ({
    ...
    contentHashIdx: index('crawl_results_content_hash_idx').on(table.contentHash),
