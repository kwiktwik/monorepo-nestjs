...
export class CrawlSchedulerService {
  ...
  async initializeSchedules(): Promise<void> {
    const activeEndpoints = await this.endpointService.findAll({ active: true });
    ...
    this.logger.log(`Initialized ${activeEndpoints.length} scheduled endpoints`);
  }
  ...
  scheduleEndpoint(endpoint: CrawlEndpoint): void {
    ...
    const cronExpression = endpoint.schedule.cron || 
      this.intervalToCron(endpoint.schedule.intervalMinutes || 60);

    // Store the scheduled task
    const scheduledTask = setInterval(async () => {
      try {
        this.logger.log(`Running scheduled crawl for endpoint: ${endpoint.name}`);
        await this.crawlerService.crawlEndpoint(endpoint.id);
        
        // Update next run time
        const nextRun = new Date(Date.now() + (endpoint.schedule.intervalMinutes || 60) * 60000);
        this.endpointService.updateNextCrawlTime(endpoint.id, nextRun);
      } catch (error) {
        this.logger.error(`Scheduled crawl failed for endpoint ${endpoint.name}:`, error);
      }
    ...
    this.logger.log(`Scheduled endpoint ${endpoint.name} with interval: ${endpoint.schedule.intervalMinutes || 60} minutes`);
  }
