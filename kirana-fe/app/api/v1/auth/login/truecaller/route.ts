...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        const { phoneNumber, code, code_verifier, client_id } = body as LoginTruecallerRequest;

        // Validate required fields
        if (!phoneNumber || !code || !code_verifier || !client_id) {
            throw new Error("Missing required fields: phoneNumber, code, code_verifier, client_id");
        }
        ...
        return NextResponse.json({
            success: false,
            error: error instanceof Error ? error.message : "Internal server error during Truecaller login",
        } as UnifiedLoginResponse, { status: 500 });
    }
}
