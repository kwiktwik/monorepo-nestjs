<!-- Source: https://developer.phonepe.com/payment-gateway/autopay/standard-checkout/redemption-order-status -->
...
| `paymentFlow` | Array | Provides the details of the subscription. |
| `MetaInfo` | Array | Contains metadata provided by the merchant. |
| `notifiedAt` | DateTime | Time (in milliseconds) when the customer got the notification about the redemption. |
...
In next section, how to deactivate an active autopay subscription that is no longer required.
