<!-- Source: https://razorpay.com/docs/payments/payment-methods/upi/wallets-on-upi -->
...
- **Payment Webhook**: The payment entity will now include the payer account type details. Refer to the [Payment Authorized Payload](/docs/webhooks/payments/#payment-authorized)  .
- **Fetch Payment API**: The payment entity will now include the payer account type details. Refer to the [Fetch API sample code](/razorpay-docs-md/api/payments.md#expanded-upi)  .
- **On the Dashboard** :
  ...
  2. The payment drawer will display the following information for wallet payments on UPI:
     ...
     ![Image shows Wallet payment via UPI on Razorpay Dashboard](https://razorpay.com/docs/payments/payment-methods/upi/build/browser/assets/images/wallet-upi-dashboard.jpg)
...
You will not be able to accept wallet payments on UPI if you fall under the [**businesses on Customer fee bearer (CFB)**](/razorpay-docs-md/dashboard/account-settings/configuration/convenience-fees.md) category.
