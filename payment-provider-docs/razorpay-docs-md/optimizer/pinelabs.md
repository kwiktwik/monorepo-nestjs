<!-- Source: https://razorpay.com/docs/payments/optimizer/pinelabs -->
...
1. [Write to your PineLabs Plural Account Manager](/razorpay-docs-md/optimizer/pinelabs.md#email-format)
...
3. Pine Labs Plural supports only the static callback URL (merchant\_return\_url). You need to share Razorpay's dynamic callback URL to whitelist. Share the following URL:
...
4. To pass your unique **Order ID** or **Receipt** for every order, write to your Relationship Manager to show the `udf_data.udf_field_1` parameter in PineLabs Dashboard and customise report. Know more about [how to send your order id to PineLabs](/razorpay-docs-md/optimizer/pinelabs.md#send-receipt-order-id-to-external-gateway)   .
...
The dynamic URL pattern needs to be whitelisted for PineLabs, or the payments will fail. To whitelist the dynamic URL pattern for PineLabs, you should communicate the requirements to the PineLabs account manager via email in a specific manner. Know more about [how to communicate via email](/razorpay-docs-md/optimizer/pinelabs.md#email-format).
...
4. Select **PineLabs** in the list of gateways available and click **Next**.
   ...
   ![Add PineLabs](https://razorpay.com/docs/payments/optimizer/build/browser/assets/images/add-pinelabs.jpg)
5. Enter the provider name and description and click **Next**.

   ![Add PineLabs Provider](https://razorpay.com/docs/payments/optimizer/build/browser/assets/images/add-provider-pinelabs.jpg)
...
7. Select the payment methods you want to enable for PineLabs and click **Submit**.
   ...
   ![Add Access Code](https://razorpay.com/docs/payments/optimizer/build/browser/assets/images/add-access-code.jpg)
...
Follow the email format given below to communicate the [prerequisites](/razorpay-docs-md/optimizer/pinelabs.md#prerequisites) or any other requirements to the PineLabs account manager:
...
You might be generating a unique Order ID or Receipt for every order which can be passed to PineLabs via [Orders API](/razorpay-docs-md/api/orders.md#orders-api).
...
5. Enter the value 100 in the **Route field**, select **PineLabs** in the **Payment Via** field, and click **Next**.
   ...
   ![PineLabs target Provider](https://razorpay.com/docs/payments/optimizer/build/browser/assets/images/pinelabs-target-provider.jpg)
6. Click **Publish Rule**.

   ![PineLabs Publish Rule](https://razorpay.com/docs/payments/optimizer/build/browser/assets/images/pinelabs-publish-rule.jpg)
