<!-- Source: https://razorpay.com/docs/payments/optimizer/cashfree -->
...
1. [Write to your Cashfree Relationship Manager](/razorpay-docs-md/optimizer/cashfree.md#email-format-for-cashfree)

   and mention that you are using Optimizer to handle sensitive card data. Request to enable the following:
   - Seamless mode for your account.
   - Whitelist the domain/URL `https://api.razorpay.com/`.
2. Copy Razorpay in the email and we will provide the supporting document from our end. Know more about the [email format for Cashfree](/razorpay-docs-md/optimizer/cashfree.md#email-format-for-cashfree)   .
3. To pass your unique **Order ID** or **Receipt** for every order, write to your Relationship Manager to show the `orderNote` parameter in the Cashfree Dashboard and customise report. Know more about [how to send your order id to Cashfree](/razorpay-docs-md/optimizer/cashfree.md#send-receipt-order-id-to-external-gateway)   .
4. On the Cashfree Dashboard, select the `v0` version for payment success, pending and fail state events.
5. For real time sync of refund status on Optimizer, please configure refund webhooks on your Cashfree dashboard. Endpoint URL:

   <https://api.razorpay.com/v1/callback/cashfree>
...
4. Select **Cashfree** in the list of gateways available and click **Next**.
   ...
   ![Add Cashfree](https://razorpay.com/docs/payments/optimizer/build/browser/assets/images/add-cashfree.jpg)
5. Enter the provider name and description and click **Next**.

   ![Add cashfree Provider Name](https://razorpay.com/docs/payments/optimizer/build/browser/assets/images/cashfree-provider-name.jpg)
6. Enter your App ID and App secret Key.
7. Select the payment methods you want to enable for Cashfree and click **Submit**.

   ![Add App key secret cashfree](https://razorpay.com/docs/payments/optimizer/build/browser/assets/images/add-app-key-secret.jpg)
...
Cashfree supports [Third-Party Validation (TPV)](/razorpay-docs-md/optimizer/third-party-validation.md#supported-bank-gateways-payment-gateways-and-payment-methods).

## Email Format for Cashfree

Follow the email format given below to communicate the [prerequisites](/razorpay-docs-md/optimizer/cashfree.md#prerequisites) or any other requirements to the Cashfree Account Manager:

Dear `Account Manager`,
We are using “Optimizer” as a technology service provider to manage our integration with Cashfree for account `<Merchant/Account ID>`.
...
You might be generating a unique Order ID or Receipt for every order which can be passed to Cashfree via [Orders API](/razorpay-docs-md/api/orders.md#orders-api).
...
5. Enter the value 100 in the **Route field**, select **Cashfree** in the **Payment Via** field, and click **Next**.
   ...
   ![Cashfree target Provider](https://razorpay.com/docs/payments/optimizer/build/browser/assets/images/cashfree-target-provider.jpg)
6. Click **Publish Rule**.

   ![Cashfree-Publish Rule](https://razorpay.com/docs/payments/optimizer/build/browser/assets/images/cashfree-publish-rule.jpg)
