<!-- Source: https://razorpay.com/docs/payments/optimizer/faqs -->

## General

1. Which types of Razorpay Checkout does Optimizer support?

Optimizer is compatible with all types of Razorpay Checkout, including [Standard](/razorpay-docs-md/payment-gateway/web-integration/standard.md), [Custom](/razorpay-docs-md/payment-gateway/web-integration/custom.md), and [Server-to-Server (S2S)](/razorpay-docs-md/payment-gateway/s2s-integration.md) integrations.
...
Optimizer supports a wide range of payment gateways and aggregators, including PayU, Pinelabs, Billdesk, Cashfree, Paytm, and many others. Find the complete [list of supported gateways](/razorpay-docs-md/optimizer/add-payment-providers.md#supported-payment-providers).

4. What payment modes are supported by Optimizer for different payment gateways?

Optimizer supports a range of payment modes, including cards, UPI, Netbanking, wallets, and EMI, providing a comprehensive payment solution. Optimizer also facilitates seamless refund and settlement processes, enhancing the overall payment experience. Know more about [supported gateways and methods](/razorpay-docs-md/optimizer/supported-gateways-aggregators.md).
...
2. Follow the [steps to add a payment gateway](/razorpay-docs-md/optimizer/add-payment-providers.md)   .

**Watch Out!**

- Ensure that the required prerequisites for your Merchant ID (MID) are enabled/configured by the respective gateways.
- Please note that the prerequisites may vary for each gateway. Know more about [prerequisites for various gateways](/razorpay-docs-md/optimizer/add-payment-providers.md#supported-payment-providers)  .
...
When Optimizer is enabled for your account, an additional parameter `gateway_provider` is included in the payment fetch response and webhook events. This parameter specifies the payment gateway through which the payment was made. Know more about [Payment API and Webhook events](/razorpay-docs-md/optimizer/get-started.md#payments-api).
...
10. Payments are failing for me with the error code **GATEWAY\_ERROR** with an internal error of **GATEWAY\_ERROR\_AUTHENTICATION\_FAILED** and the description “Payment processing failed due to error at bank or wallet gateway”. How should I correct it?
...
The custom identifier needs to be passed in the Payments or Orders API. Know more about [custom identifiers](/razorpay-docs-md/optimizer/create-custom-rule.md#custom-identifiers).
...
- [Normal Refunds](/razorpay-docs-md/refunds/normal.md)
  ...
  : For normal refunds, the refund is initiated with the respective payment gateway and depends on the availability of funds in the gateway's balance.
- [Instant Refunds](/razorpay-docs-md/refunds/instant.md)

  : For instant refunds, If Razorpay processes the payment, we initiate an instant refund through Razorpay. If another payment gateway processes the payment, we initiate the refund through that specific gateway.
