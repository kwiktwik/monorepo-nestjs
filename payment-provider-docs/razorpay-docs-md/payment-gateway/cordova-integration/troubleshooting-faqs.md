<!-- Source: https://razorpay.com/docs/payments/payment-gateway/cordova-integration/troubleshooting-faqs -->
...
Order creation is the primary step of the Razorpay payment flow. When a customer clicks the pay button on your app, an Order is created and a corresponding order\_id is generated in the response. This order\_id must be passed to the Razorpay Checkout options added in your Capacitor app.
Know more about [Orders](/razorpay-docs-md/api/orders.md).
...
1. Use [Rosetta 2](https://support.apple.com/en-us/HT211861)

   for launching the app on your Mac.
...
3. Look for the `<plugin>` tag that includes the Razorpay SDK plugin. The version number is represented by `x.y.z`.
