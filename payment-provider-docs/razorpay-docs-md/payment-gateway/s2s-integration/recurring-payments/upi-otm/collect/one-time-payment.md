<!-- Source: https://razorpay.com/docs/payments/payment-gateway/s2s-integration/recurring-payments/upi-otm/collect/one-time-payment -->

You should perform the following steps to create and charge your customer a one time payment:

1. [Create an order to charge the customer](/razorpay-docs-md/payment-gateway/s2s-integration/recurring-payments/upi-otm/collect/one-time-payment.md#31-create-an-order-to-charge-the-customer)
2. [Create a one time payment](/razorpay-docs-md/payment-gateway/s2s-integration/recurring-payments/upi-otm/collect/one-time-payment.md#32-create-a-one-time-payment)
...
Once you have generated an `order_id`, use it with the `token_id` to create a one-time payment. After the payment is complete (authorised or failed), the respective token will move to the `cancelled` state, and you can no longer use the token. You will get the [token.cancelled](/razorpay-docs-md/api/payments/recurring-payments/webhooks.md#token-cancelled) webhook notification for the same.
